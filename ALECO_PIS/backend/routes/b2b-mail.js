...
router.get('/b2b-mail/contacts', requireStaff, async (req, res) => {
    try {
...
router.post('/b2b-mail/contacts', requireStaff, async (req, res) => {
    try {
        const body = req.body || {};
        const hadNumericId =
            body.id != null &&
...
router.put('/b2b-mail/contacts/:id', requireStaff, async (req, res) => {
    try {
...
router.post('/b2b-mail/contacts/:id/send-verification', requireStaff, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid id' });
...
router.get('/b2b-mail/messages', requireStaff, async (req, res) => {
    try {
...
router.get('/b2b-mail/messages/:id', requireStaff, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid id' });
...
router.post('/b2b-mail/messages/preview-recipients', requireStaff, async (req, res) => {
    try {
...
router.post('/b2b-mail/messages/:id/preview-recipients', requireStaff, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid id' });
        const [mRows] = await pool.execute('SELECT * FROM aleco_b2b_messages WHERE id = ? LIMIT 1', [id]);
...
router.post('/b2b-mail/messages/draft', requireStaff, async (req, res) => {
    try {
...
router.put('/b2b-mail/messages/:id', requireStaff, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid id' });
...
router.post('/b2b-mail/messages/:id/send', requireStaff, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid id' });
...
router.post('/b2b-mail/messages/:id/retry', requireStaff, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: 'Invalid id' });
...
router.get('/b2b-mail/templates', requireStaff, async (req, res) => {
    try {
        const [rows] = await pool.execute(
...
router.post('/b2b-mail/templates', requireStaff, async (req, res) => {
    try {
        const body = withActorFromReq(req, req.body || {});
...
router.get('/b2b-mail/inbound', requireStaff, async (req, res) => {
    try {
 ...
 */
router.post('/b2b-mail/inbound/refresh', requireStaff, async (req, res) => {
    try {
        const messageId = req.query.messageId != null && String(req.query.messageId).trim() !== '' ? Number(req.query.messageId) : null;
