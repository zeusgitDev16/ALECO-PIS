...
const ServiceMemoCard = ({ memo, onView, onEdit, onClose, onPrint, currentUserEmail }) => {
  ...
  return (
    <div className="service-memo-card">
      ...
      <div className="service-memo-card-body">
        <h3 className="service-memo-ticket-id">
          #{controlNumber} - {memo.ticket_id}
        </h3>

        <div className="service-memo-customer-info">
          <p className="service-memo-customer-name">{fullName}</p>
          {memo.phone_number && <p className="service-memo-phone">{memo.phone_number}</p>}
        </div>
        ...
        {memo.work_performed && (
          <div className="service-memo-preview">
            <p className="preview-label">Action taken:</p>
            <p className="preview-text">
              {memo.work_performed.substring(0, 100)}
              {memo.work_performed.length > 100 ? '...' : ''}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="service-memo-card-actions">
        ...
        {isOwner && memo.memo_status !== 'closed' && (
          <button type="button" className="service-memo-action-btn service-memo-action-btn--edit" onClick={() => onEdit(memo.id)} title="Edit memo">
            Edit
          </button>
        )}

        <button type="button" className="service-memo-action-btn service-memo-action-btn--print" onClick={() => onPrint(memo)} title="Print memo">
          Print
        </button>

        {isOwner && memo.memo_status === 'saved' && (
          <button type="button" className="service-memo-action-btn service-memo-action-btn--close" onClick={() => onClose(memo.id)} title="Close memo">
            Close
          </button>
        )}
      </div>
